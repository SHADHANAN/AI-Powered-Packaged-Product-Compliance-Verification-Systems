...
def test_ocr_service_empty_or_nonexistent_image():
    """Ensure OCR service raises BadRequestException for invalid paths."""
...
def test_ocr_service_valid_image(monkeypatch):
    ...
    try:
        ...
        # Mock Tesseract command detection.
        monkeypatch.setattr(
            ocr_service.service,
            "get_tesseract_command",
            lambda: "mock_tesseract",
        )
        ...
        # Mock Tesseract subprocess execution.
        monkeypatch.setattr(
            subprocess,
            "run",
...
def test_ocr_service_handles_missing_tesseract_gracefully(monkeypatch):
    """Test OCR service returns fallback when Tesseract is unavailable."""
