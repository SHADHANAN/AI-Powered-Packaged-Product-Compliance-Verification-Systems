...
def extract_text_from_image(image_path: str) -> str:
    ...
    2. Executes Tesseract OCR via subprocess (with graceful fallback if binary is not installed).
    ...
    if not tesseract_cmd:
        logger.warning(
            "Tesseract OCR binary not found on the host system. Returning empty OCR text fallback."
        )
        return ""

    # 3. Save preprocessed image to a temporary file for Tesseract execution
    ...
    try:
        ...
        if result.returncode != 0:
            logger.warning(f"Tesseract returned non-zero code {result.returncode}: {result.stderr}")
            return ""

        cleaned_text = result.stdout.strip()
        logger.info(f"Successfully extracted {len(cleaned_text)} characters of OCR text")
        return cleaned_text

    except subprocess.TimeoutExpired:
        logger.error(f"Tesseract OCR timed out after {settings.OCR_TIMEOUT_SECONDS}s")
        raise InternalServerErrorException("OCR processing timed out")
    except Exception as exc:
        logger.error(f"Unexpected error during OCR text extraction: {exc}", exc_info=True)
        raise InternalServerErrorException("Failed to extract text from product image")
    finally:
